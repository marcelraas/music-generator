...
from music_generator.synthesizer.oscillators import SquareOscillator, AliasingSquareOscillator
...
def monophonic_scale(n_notes,
    ...
    base_osc = base_osc(sampling_info)
    ...
    y = np.concatenate(list(map(
        lambda f: chord_generator.generate_chord(cisd.generate_chord(f),
                                                 note_duration * 16, amp,
                                                 phase=0), notes)))
    ...
    lead_generator = FilteredOscillator(sampling_info, 10000, filter_type="lowpass", base_generator=base_osc, order=1)
    # lead_generator = SquareOscillator(sampling_info)

    y_lead = np.concatenate(list(map(
        lambda f: lead_generator.generate_note(f,
                                               note_duration,
                                               amp,
                                               phase=0), notes)))
